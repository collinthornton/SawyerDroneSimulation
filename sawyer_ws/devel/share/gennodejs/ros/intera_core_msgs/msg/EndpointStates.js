...
class EndpointStates {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.states.forEach((val) => {
      bufferOffset = EndpointState.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new EndpointStates(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.states[i] = EndpointState.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new EndpointStates(null);
    ...
    if (msg.states !== undefined) {
      ...
      for (let i = 0; i < resolved.states.length; ++i) {
        resolved.states[i] = EndpointState.Resolve(msg.states[i]);
      }
    }
    else {
      resolved.states = []
